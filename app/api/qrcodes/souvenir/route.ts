...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!existing.hadir) {
      return NextResponse.json({ error: "Belum hadir" }, { status: 412 });
    }
    if (existing.souvenir) {
      return NextResponse.json(
        { error: "Souvenir already taken", data: existing },
        { status: 409 },
      );
    }
    const [updated] = await db
      .update(qrcode)
      .set({ souvenir: true, updatedAt: new Date() })
      .where(eq(qrcode.token, token))
      .returning();
