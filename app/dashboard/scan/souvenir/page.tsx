...
export default function ScanSouvenirPage() {
  ...
  const fetchRecent = async () => {
    try {
      ...
      const res = await fetch("/api/qrcodes/recent?type=souvenir&limit=5", {
        cache: "no-store",
      });
  ...
  const handleSubmit = async () => {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/qrcodes/souvenir", {
        method: "POST",
      ...
      if (!res.ok) {
        if (res.status === 409) {
          toast.info("Souvenir sudah diambil (diabaikan)");
        } else if (res.status === 412) {
          toast.error("Belum hadir, tidak bisa scan souvenir");
        } else {
          throw new Error(data?.error || "Gagal");
        }
      } else {
        toast.success("Souvenir ditandai diambil");
      }
      setTokenValue("");
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Mode input saja (dukungan pemindai keyboard). Wajib hadir sebelum
            scan souvenir.
          </p>
        </div>
        ...
        >
          ...
          <div className="flex gap-2">
            <Button type="submit" disabled={busy}>
              {busy ? "Memproses..." : "Tandai Souvenir"}
            </Button>
          </div>
        </form>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50">
                <tr>
                  ...
                  <th className="text-left p-2">Hadir</th>
                </tr>
              </thead>
              <tbody>
                {recentLoading ? (
